...
const QTableSearch = ({
  ...
  useEffect(() => {
    ...
    if (tableSearchRef.current) {
      observer.observe(tableSearchRef.current);
    }

    return () => {
      if (tableSearchRef.current) {
        observer.unobserve(tableSearchRef.current);
      }
    };
...
export default QTableSearch;
QTableSearch.displayName = "QTableSearch";
