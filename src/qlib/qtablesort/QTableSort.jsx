...
const QTableSort = ({
  ...
  const tableSortRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (tableSortRef.current) {
      observer.observe(tableSortRef.current);
    }

    return () => {
      if (tableSortRef.current) {
        observer.unobserve(tableSortRef.current);
      }
    };
...
QTableSort.displayName = "QTableSort";
